...
min_temperature = parsed_lines.filter(lambda line: "TMIN" in line[1])

min_temperature_station = min_temperature.map(lambda x: (x[0], x[2]))

min_temperature_till_date_station = min_temperature_station.reduceByKey(lambda x, y: min(x,y))
