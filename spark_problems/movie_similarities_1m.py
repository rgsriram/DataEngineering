...
data = sc.textFile("s3n://sriram-spark-examples/ml-1m/ratings.dat")
...
if (len(sys.argv) > 1):
    ...
    filteredResults = moviePairSimilarities.filter(lambda pair: \
        ...
        and pair[1][0] > scoreThreshold and pair[1][1] > coOccurenceThreshold)

    # Sort by quality score.
    results = filteredResults.map(lambda pair: (pair[1], pair[0])).sortByKey(ascending = False).take(10)
